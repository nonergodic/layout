...
>(
  ...
  internalserialize(layout, data, cursor);
...
function internalserialize(
  layout: Layout,
  data: any,
  cursor: Cursor,
) {
  if (isLayoutItem(layout))
    serializeItem(layout as LayoutItem, data, cursor);
  else
    for (const item of layout)
      try {
        serializeItem(item, data[item.name], cursor);
      }
      catch (e: any) {
...
function serializeItem(item: LayoutItem, data: any, cursor: Cursor) {
  switch (item.binary) {
    case "int":
    case "uint": {
      const value = (() => {
    ...
    case "bytes": {
      ...
      if ("layout" in item) {
        ...
        internalserialize(item.layout, layoutData, cursor);
      }
      else {
        const { custom } = item;
    ...
    case "array": {
      ...
      for (let i = 0; i < data.length; ++i)
        internalserialize(item.layout, data[i], cursor);
